...
(define (key n)
  (let ((e (second(get-nth-quadruple n))) (f (third(get-nth-quadruple n))))
    (map (mod-by 27) (list (a1 e f) (b1 e f) (c1 e f) (a2 e f) (b2 e f) (c2 e f) (a3 e f) (b3 e f) (c3 e f)))
    ))
  ;)
...
(define (message->codes message)
  (map (mod-by 27) (map char-to-int (string->list message))))

(define (char-to-int a)
  (if (= (char->integer a) 32)
      0
      (- (char->integer a) 96)))
...
(define (codes->message codes)
  (list->string (map int-to-char codes)))

(define (int-to-char a)
  (if (= a 0)
      (integer->char (+ a 32))
      (integer->char (+ a 96))
  ))
...
(define (extend-key key size)
  (let ((repeat (quotient size (length key))) (remain (remainder size (length key))))
    (append (repeat-key key repeat '()) (remainder-key key remain '()))))

(define (repeat-key key n acc)
  (if (= n 0)
      acc
      (repeat-key key (- n 1) (append acc key)))
  )

(define (remainder-key key n acc)
  (if (= n 0)
      acc
      (remainder-key (cdr key) (- n 1) (append acc (list (car key)))))
  )
...
(define (endec code key x)
  (if (= x 1)
      (map (mod-by 27) (map + code (extend-key key (length code))))
      (map (mod-by 27) (map - code (extend-key key (length code)))))
  )


(define encrypt-codes
  (λ (message key) (endec message key 1))
  )
(define decrypt-codes
  (λ (crypted key) (endec crypted key 0))
  )
...
(define (endec-string code key x)
  (if (= x 1)
      (let ((msg (message->codes code))) (codes->message (encrypt-codes msg key)))
      (let ((msg (message->codes code))) (codes->message (decrypt-codes msg key)))
      ))


(define encrypt-message
  (λ (message key) (endec-string message key 1))
  ;(λ (message key) 
  ;(let ((msg (message->codes message))) (codes->message (encrypt-codes msg key)))
 ; )
)
(define decrypt-message
  (λ (code key) (endec-string code key 0))
  )
