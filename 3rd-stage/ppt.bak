...
;     |-2 1 2|·|4| = | 8|
;     |-2 2 3| |5|   |17|
(define (dot-product2 X)
  (λ (Y)
    (foldl + 0 (map * X Y))))

(define (multiply M V)
  (map (dot-product2 V)  M))
...
;(apply-functional-transformations (list + - *) '(1 1 2 3))
;(apply-functional-transformations (list Q1 Q2 Q3) '(1 1 2 3))
;(apply Q1  '(1 1 2 3))
...
(define (get-matrix-list Mt F1 F2 F3)
  (map (λ (x) (cond ((= x 1) F1) ((= x 2) F2) (else F3))) Mt)
  )

(define (multiply2 L)
  (λ (T)
    (map (dot-product2 T) L)))

(define (apply-functional-transformations2 Fs tuple)
  (foldl (λ (f tuple) (apply f tuple)) tuple Fs))
...
(define get-nth-tuple
  (λ (n F1 F2 F3 Ts Mt)
    (if (= n 1)
        Ts
        (if (= (length Ts) 3)
            (foldl multiply Ts (get-matrix-list Mt F1 F2 F3))
            (apply-functional-transformations2 (get-matrix-list Mt F1 F2 F3) Ts)
            ;1
    ))))
;(list reverse ((curry map) add1) cdr)
;(get-matrix-list (get-transformations 40) Q1 Q2 Q3)
; TODO
...
(define get-nth-ppt-from-matrix-transformations
  (λ (n) (get-nth-tuple n T1 T2 T3 '(3 4 5)(get-transformations n))))

;(get-nth-ppt-from-matrix-transformations 64)
;(get-nth-ppt-from-matrix-transformations 15)
...
(define get-nth-quadruple
  (λ (n) (get-nth-tuple n Q1 Q2 Q3 '(1 1 2 3) (get-transformations n))))
...
(define get-nth-ppt-from-GH-quadruples
  (λ (n) (list (* (first (get-nth-quadruple n)) (fourth (get-nth-quadruple n))) (* 2 (* (second (get-nth-quadruple n)) (third (get-nth-quadruple n))))
               (+ (* (second (get-nth-quadruple n)) (second (get-nth-quadruple n))) (* (third (get-nth-quadruple n)) (third (get-nth-quadruple n)))))))
